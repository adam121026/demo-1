...
class Error {
    ...
    string OPEN_ERROR = "Error in opening the file/folder ";
    string PATH_ERROR = "No such directory found";
    ...
    void setAllPermissions(string& path, string& obj_type) {
        ...
        if (stat(path.c_str(), &object_info) == 0) {
            for (int i = 0; i < 3; i++) {
                ...
                write_per[i] = (object_info.st_mode & permission_mode[i][0]) != 0;
                execute_per[i] = (object_info.st_mode & permission_mode[i][0]) != 0;
            }
        } else {
            Error err;
            err.displayError(err.OPEN_ERROR);
        }
    }
...
class Directory_info {
    ...
    bool checkfordir(string path) {
        ...
        error.displayError(error.PATH_ERROR);
        return false;
    }
